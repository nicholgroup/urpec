...
% Version history
% v2: doesn't exist
% v3: belongs with urpec_v3. Calls urpec_writeBlock_v3, which ssumes that all patterns are in the same
...
head.alignPrev.str=['AlignSys P',lineChar];
...
for i=1:length(entities)
    ...
    if strmatch(entities(i).type,'write')
        writeBlock=[writeBlock urpec_writeBlock_v3(entities(i)),lineChar,'*',lineChar];
    end
