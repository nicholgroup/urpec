...
%layer numbering starts at 2 and goes up with patterns created with urpec
%JMN
try
    layerNum = config.layer;
catch
    layerNum=1;
end
...
strline1 = ['lev_1 ' num2str(layerNum+1) ' w    0,0    1500    ' config.mag '    ' config.spacing{1} '    ' config.spacing{2} '    ' aperture '     ' current];
tot_str_s = strline1;
for i=1:length(ctab)
